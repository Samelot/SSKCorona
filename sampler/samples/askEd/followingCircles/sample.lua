...
function public.run( group )
	...
	--
	local moveSpeed = 200
	local distanceBetweenCircles = 35
	...
	local function onTouch( self, event )
		if( event.phase == "began" ) then
			...
			-- Calculate the distance and speed for the rest.
			--
			for i = 2, #circles do
				-- Where to move to calc
				local vec2 = normVec( vec )				
				vec2 = scaleVec( vec2, (i-1) * distanceBetweenCircles )
				vec2 = addVec( tvec, vec2 )

				-- Speed calc
				local vec3 = subVec( vec2, circles[i] )
				local len = lenVec( vec3 )
				local time = 1000 * len / moveSpeed

				-- Move
				transition.to( circles[i], { x = vec2.x, y = vec2.y, time = time })
...
function public.about()
	local altName = "Following Circles (MAY 2015)"
	local description = 
	'<font size="16" color="SteelBlue">Following Circles (MAY 2015)</font><br><br><br>' ..
    'From this post: <a href = "http://bit.ly/ssk_following_circles">CLICK HERE</a><br><br>'
